...
def run_baseline_pair(input_path: str, setting_keys: List[str] = None):
    ...
    for setting_key, setting_data in settings.items():
        ...
        if vectorization == COOC:
            ...
            cv_content1_train = df_train_laser.toarray()
            ...
            df_train = pd.read_json("embeddings_train.json",lines=True)
            df_train_laser = df_train.loc[df_train["lang_1"].isin(train_langs)]
            emb_1 = np.array(df_train_laser['emb_1'].values)
            emb_2 = np.array(df_train_laser['emb_2'].values)
            X = np.array(df_train_laser['abs_diff'].values)

            normalizer = preprocessing.Normalizer().fit(list(X))
            train_data_embeddings = normalizer.transform(list(X))

            #X = np.array([emb_1,emb_2,abs_diff]).T
            # Extract target i.e. iris species
            Y = np.array(df_train_laser['label'].values)
        else:
        ...
        rs = GridSearchCV(estimator=est, param_grid=parameters, scoring="f1_macro", cv=5,
                                n_jobs=-1, verbose=10, refit=True)

        if vectorization!=LASER:
            rs.fit(train_data_embeddings, train_data["label"].astype(int))
        else:
            rs.fit(list(train_data_embeddings), list(Y))
        ...
        # Run predictions for Magellan features
            ...
            for lang in test_langs:
                ...
                output_and_store_results(setting_data, settings_name, category, train_langs_str, lang,
                    ...
                    for lang in test_langs:
                        # Subset the test data
                        test_data_laser = pd.read_json("embeddings_test.json",lines=True)
                        test_data_lang = test_data_laser.loc[test_data_laser['lang_1'] == lang]
                        
                        emb_1 = np.array(test_data_lang['emb_1'].values)
                        emb_2 = np.array(test_data_lang['emb_2'].values)
                        abs_diff = np.array(test_data_lang['abs_diff'].values)

                        normalizer = preprocessing.Normalizer().fit(list(abs_diff))
                        test_data_embeddings = normalizer.transform(list(abs_diff))
                        # Normalize Features
                        #test_data_embeddings_lang = normalizer.transform(test_data_embeddings_lang)

                        # Prediction and computation of metrics to measure performance of model
                        pred = rs.best_estimator_.predict(list(test_data_embeddings))
                        scores_per_lang[lang] = compute_metrics(
                            {"labels": test_data_lang["label"], "predictions": pred}).get("f1")
                        output_and_store_results(setting_data, settings_name, category, train_langs_str, lang,
                                                scores_per_lang[lang], "", str(rs.best_params_), input_path, pred)
